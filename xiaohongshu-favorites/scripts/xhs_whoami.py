...
# ///
"""
Best-effort helper: detect the currently logged-in XiaoHongShu profile id.

This uses Playwright + a DOM heuristic (header profile link). If detection fails,
pass --profile-id explicitly to other scripts.
...
def cookiecloud_cookies_for_playwright(domain_suffix: str) -> list[dict[str, Any]] | None:
    ...
    for c in raw_cookies:
        ...
        # Optional fields Playwright understands.
...
def detect_profile_id_from_state(page) -> str | None:
    # Avoid DOM coupling: look for "/user/profile/<id>" inside hydrated state.
    href = page.evaluate(
        """() => {
...
def detect_profile_id(page) -> str | None:
    # Heuristic 1: header profile link (fast when present).
    href = page.evaluate(
        """() => {
    ...
    # Heuristic 2: any profile link in DOM (UI may move it out of header).
    ...
    # Heuristic 3: search hydrated state for a profile link.
...
def main() -> int:
    parser = argparse.ArgumentParser(description="Detect logged-in XiaoHongShu profile id")
    ...
    with sync_playwright() as p:
        ...
        profile_id = detect_profile_id(page)
        if not profile_id:
            click_me(page)
            try:
                page.wait_for_load_state("networkidle", timeout=15000)
        ...
        # Cleanup early to avoid leaving Chrome running.
        context.close()
        browser.close()

    if args.json:
        print(json.dumps({"platform": "xiaohongshu", "profile_id": profile_id}, ensure_ascii=False))
        return 0 if profile_id else 2

    if profile_id:
        print(profile_id)
        return 0

    print("Failed to detect profile id. Pass --profile-id explicitly.", file=sys.stderr)
    return 2
